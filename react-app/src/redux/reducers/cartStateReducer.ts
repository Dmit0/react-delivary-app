import {SET_MEAL_TO_CART,REMOVE_ITEM_FROM_CART,REMOVE_ONE_MEAL_FROM_CART,SET_MEAL_FROM_LOALESTORAGE_TO_CART,CLEAN_CART,cartActionTypes,CartState} from '../types/cartTypes'
...
export const cartReducer=(state=initialState,action:cartActionTypes):CartState=>{
    ...
    switch (action.type){
        ...
        let Item = state.cart.find(item=>(
            ...
            // let prevItems=state.cart.filter(item=>(
            //     item._id!==action.set_cart_meal._id
            // ))
            // Item.count+=1
            // return {...state,cart:[...prevItems,Item]}
        ...
        case REMOVE_ONE_MEAL_FROM_CART:
            // let DelItem = state.cart.find(item=>(
            //     item._id===action.remove_one_cart_meal._id
            // ))
            // if(DelItem===undefined || DelItem.count===1){
            //     return {...state,cart:state.cart.filter((item)=>item._id!==action.remove_one_cart_meal._id)}
            // }
            // let prevDelItems=state.cart.filter(item=>(
            //     item._id!==action.remove_one_cart_meal._id
            // ))
            // DelItem.count-=1
            // return {...state,cart:[...prevDelItems,DelItem]}
