...
export const RegistrationPopup: React.FC<RegistrationProps> = ({
  ...
  const regions = useSelector(getSelectRegions)
  ...
  useEffect(() => {
    if (isAuthStepContinue) {
      fetchRegions(firstUserCountry?.code);
    }
  }, [ isAuthStepContinue, firstUserCountry, fetchRegions ]);
...
export const Item = ({ register, current_country_code, changeHandler }: any) => {
  return (
    <>
      <input name='telephone' disabled={ !current_country_code } value={ current_country_code ? current_country_code : ' select country ' }
             onChange={ changeHandler } ref={ register({
        required: true,
        minLength: 11,
        maxLength: 15,
      }) }/>
